...
module Homebrew
  ...
  class Test
    ...
    def initialize(argument, tap: nil, skip_setup: false, skip_homebrew: false, skip_cleanup_before: false, skip_cleanup_after: false, bintray_domain: HOMEBREW_BOTTLE_DOMAIN)
      @hash = nil
      @url = nil
      @formulae = []
      @added_formulae = []
  ...
  def test_ci_upload(tap, bintray_bottle_domain)
    ...
    remote = if github_token
      "https://#{github_token}@github.com/#{git_name}/homebrew-#{tap.repo}.git"
    else
      "git@github.com:#{git_name}/homebrew-#{tap.repo}.git"
    end
    git_tag = if pr
      "pr-#{pr}"
    ...
    if git_tag
      if ARGV.include?("--dry-run")
        puts "git push --force #{remote} :refs/tags/#{git_tag}"
      else
